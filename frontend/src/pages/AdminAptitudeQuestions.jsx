...
import AptitudeQuestionForm from "../components/AptitudeQuestionForm.jsx";
import Loader from "../components/Loader.jsx";
import useAuth from "../hooks/useAuth.js";
import {
  fetchAdminAptitudeQuestions,
  createAdminAptitudeQuestion,
  updateAdminAptitudeQuestion,
  deleteAdminAptitudeQuestion
} from "../services/adminAptitudeService.js";
...
const AdminAptitudeQuestions = () => {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      try {
        const response = await fetchAdminAptitudeQuestions({ page, limit, query });
  ...
  const persistQuestion = async (payload) => {
    ...
    try {
      if (selectedQuestion) {
        await updateAdminAptitudeQuestion(selectedQuestion._id, payload);
      } else {
        await createAdminAptitudeQuestion(payload);
      }
      setIsFormOpen(false);
      setSelectedQuestion(null);
      setPage(1);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setIsLoading(true);
      await deleteAdminAptitudeQuestion(id);
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-950">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-bold text-slate-950 dark:text-white">Aptitude Questions</h1>
              <p className="mt-2 max-w-2xl text-sm text-slate-600 dark:text-slate-400">Add, edit, and manage aptitude questions available to students.</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-950">
              ...
              {isLoading ? (
                ...
                <div className="p-6 text-sm text-slate-600 dark:text-slate-300">No aptitude questions found.</div>
              ) : (
                <div className="divide-y divide-slate-200 dark:divide-slate-800">
                  {questions.map((question) => (
                    <div key={question._id} className="grid grid-cols-[3fr_1fr_1fr_1fr] gap-4 px-4 py-4 text-sm text-slate-700 dark:text-slate-200 sm:grid-cols-[4fr_1fr_1fr_1fr]">
                      <div>
                        <p className="font-semibold text-slate-950 dark:text-white">{question.question}</p>
                        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Correct answer: {question.correctOption}</p>
                      </div>
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-950">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-bold text-slate-950 dark:text-white">Quick actions</h2>
                </div>
              </div>
              <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-400">
                Use the form above to create or update questions, then manage entries here. Search works across question text and answers.
              </p>
            </div>
            <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-950">
              <p className="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Current filters</p>
              <div className="mt-4 space-y-2 text-sm text-slate-700 dark:text-slate-200">
                <p><span className="font-semibold">Search:</span> {query || "None"}</p>
                <p><span className="font-semibold">Page size:</span> {limit}</p>
              </div>
            </div>
          </div>
        </div>
